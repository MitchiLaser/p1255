...
devenv:
	...
	@if ! venv/bin/python -c "import ps1255" 2>/dev/null; then
		echo "Installing ps1255 in editable mode";
		. venv/bin/activate;
...
lint: devenv
	. venv/bin/activate;
		...
		isort --check --diff ./src/ps1255;
		RET_Isort=$$?;
		echo -e "$(BLUE)${BOLD}BLACK${NC}$(NORM)";
		black --check --color --diff ./src/ps1255;
		RET_Black=$$?;
		echo -e "$(BLUE)${BOLD}FLAKE8${NC}$(NORM)";
		flake8 --config .flake8 ./src/ps1255;
		RET_Flake8=$$?;
